...
class ImageProjection(Node):
    ...
    def __init__(self):    
        ...
        self.image_sub = self.create_subscription(CompressedImage, '/limo/depth_camera_link/image_raw', 
                                                  self.image_color_callback, qos_profile=qos.qos_profile_sensor_data)
        
        self.image_sub = self.create_subscription(CompressedImage, '/limo/depth_camera_link/depth/image_raw', 
    ...
    def get_tf_transform(self, target_frame, source_frame):
        try:
            ...
            self.get_logger().warning(f"Failed to transform: {str(e)}")
            return None
    ...
    def image_color_callback(self, data):
        ...
        try:
            ...
            # image_color = self.bridge.compressed_imgmsg_to_cv2(data, "bgr8")
            # image_depth = self.bridge.compressed_imgmsg_to_cv2(self.image_depth_ros, "32FC1")
        ...
        odom_pose_stamped = self.tf_transform(object_location, "odom")

        # location = PoseStamped()
        # location.header.frame_id = "odom"
        # location.pose.orientation.w = 1.0
        # location.pose.position = p_camera.position

        self.object_location_pub.publish(odom_pose_stamped) 
